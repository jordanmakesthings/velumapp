...
export function AppLayout() {
  ...
  return (
    <div className="min-h-screen w-full max-w-full overflow-x-hidden bg-radial-subtle flex">
      ...
      <main className={`min-w-0 w-full max-w-full flex-1 overflow-x-hidden ${!hideNav ? "lg:ml-[220px]" : ""} ${!hideNav ? "pb-[104px] lg:pb-0" : ""}`}>
        <Outlet />
      </main>
      ...
      {!hideNav && (
        <nav className="lg:hidden fixed bottom-0 inset-x-0 z-50 h-[88px] max-w-full overflow-hidden backdrop-blur-xl bg-background/80 border-t border-accent/10 flex items-center justify-around px-6 pt-2 pb-4 safe-area-pb">
          <BottomNav />
        </nav>
      )}
    </div>
