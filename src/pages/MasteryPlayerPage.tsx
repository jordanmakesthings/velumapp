  ...
  return (
    <div className="relative flex min-h-screen w-full max-w-full flex-col overflow-x-hidden bg-background">
      ...
      <div className="safe-area-pt relative z-10 flex items-center justify-between px-4 pt-4">
        ...
        <p className="text-ui text-center text-xs tracking-wide uppercase">Mastery Class</p>
        <div className="w-9 shrink-0" />
      </div>

      <div className="relative z-10 flex flex-1 flex-col items-center overflow-y-auto px-6 pb-24">
        {/* Cover */}
        {((mc as any).player_image_url_1_1 || (mc as any).cover_image_url_16_9 || (mc as any).cover_image_url) ? (
          <div className={`mb-8 w-full max-w-sm overflow-hidden ${(mc as any).player_image_url_1_1 ? "aspect-square rounded-3xl" : "aspect-video rounded-2xl"}`}>
          ...
          <motion.div
            ...
            className="mb-8 flex h-64 w-64 items-center justify-center overflow-hidden rounded-3xl bg-card shadow-2xl"
          >
        ...
        {/* Title & theme */}
        {(mc as any).theme && (
          <p className="text-accent mb-2 text-[10px] font-sans font-medium tracking-[2.5px] uppercase">{(mc as any).theme}</p>
        )}
        <h2 className="text-display mb-1 max-w-full break-words text-center text-2xl">{mc.title}</h2>
        {mc.description && <p className="text-ui mb-8 max-w-md break-words text-center text-sm">{mc.description}</p>}

        {isGated ? (
          <div className="velum-card w-full max-w-sm p-8 text-center">
            <p className="text-accent mb-3 text-3xl">◈</p>
          ...
          <>
            ...
            <div className="mb-3 w-full max-w-sm">
              <div className="relative h-1.5 cursor-pointer rounded-full bg-surface-light" onClick={handleSeek}>
                <div className="absolute inset-y-0 left-0 rounded-full gold-gradient" style={{ width: `${progress}%` }} />
                <div className="absolute top-1/2 h-3.5 w-3.5 -translate-y-1/2 rounded-full bg-accent shadow-lg" style={{ left: `calc(${progress}% - 7px)` }} />
              </div>
            ...
            <div className="mb-8 flex items-center gap-8">
              ...
              <button onClick={togglePlay}
                className="flex h-16 w-16 items-center justify-center rounded-full gold-gradient active:scale-95 transition-transform">
                {isPlaying ? <Pause className="w-6 h-6 text-primary-foreground" /> : <Play className="ml-1 w-6 h-6 text-primary-foreground" />}
              </button>
            ...
            {activePrompts.length > 0 && (
              <div className="w-full max-w-sm">
                <div className="mb-6 h-px bg-accent/10" />
                <p className="text-accent/50 mb-2 text-[10px] font-sans tracking-[2.5px] uppercase">Reflect & Write</p>
                <p className="text-muted-foreground mb-6 text-xs font-sans">Journaling is optional — write as much or as little as you like.</p>
                {activePrompts.map((prompt: any) => (
                  <motion.div
                    ...
                    className="mb-6"
                  >
                    <p className="text-display mb-3 break-words text-base italic text-foreground/70">"{prompt.text}"</p>
                    <textarea
                      value={responses[prompt.prompt_id] || ""}
                      onChange={(e) => setResponses(r => ({ ...r, [prompt.prompt_id]: e.target.value }))}
                      rows={4}
                      placeholder="Your reflection…"
                      className="w-full rounded-xl border border-accent/10 bg-card p-4 text-sm font-sans text-foreground placeholder:text-muted-foreground/50 resize-none focus:outline-none"
                    />
                  </motion.div>
                ))}
                ...
                {!saved ? (
                  <button
                    ...
                    className="w-full rounded-xl gold-gradient py-4 text-sm font-sans font-bold text-primary-foreground active:scale-[0.98] transition-transform disabled:opacity-40"
                  >
            ...
            {completed && activePrompts.length === 0 && (
              <div className="flex flex-col items-center">
                <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: "spring" }}
                  className="mb-6 flex h-20 w-20 items-center justify-center rounded-full gold-gradient">
                  <span className="text-primary-foreground text-3xl">✓</span>
                </motion.div>
                <h3 className="text-display mb-2 text-2xl">Class complete</h3>
                <p className="text-ui mb-8 text-sm">{mc.title}</p>
                <div className="flex gap-3">
                  <button onClick={() => { setCurrentTime(0); setCompleted(false); setVisiblePromptIds(new Set()); if (audioRef.current) audioRef.current.currentTime = 0; }}
                    className="velum-card-flat flex items-center gap-2 rounded-xl px-6 py-3 text-sm font-sans text-foreground active:scale-95 transition-transform">
