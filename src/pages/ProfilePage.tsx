...
const CATEGORY_STATS = [
  { label: "Meditation", count: 12 },
  { label: "Breathwork", count: 8 },
  { label: "Tapping", count: 6 },
  { label: "Rapid Resets", count: 4 },
  { label: "Journaling", count: 3 },
];
...
export default function ProfilePage() {
  ...
  // Mock user state (will connect to auth later)
    ...
    <div className="px-4 lg:px-8 pt-14 pb-8 max-w-2xl mx-auto">
      ...
      <div className="grid grid-cols-3 gap-3 mb-8">
        {[
          ...
          { label: "Sessions", value: "33", icon: Sparkles },
          { label: "Minutes", value: "312", icon: Wind },
        ].map(({ label, value, icon: Icon }) => (
          <div key={label} className="velum-card p-4 text-center">
            <Icon className="w-4 h-4 text-accent mx-auto mb-2" />
            <p className="text-display text-2xl">{value}</p>
      ...
      <div className="velum-card-flat p-4 flex items-center gap-3 mb-8">
        <TrendingDown className="w-4 h-4 text-accent" />
        <span className="text-ui text-sm">Average stress reduction: <span className="text-accent font-medium">−47%</span></span>
      </div>
      ...
      {!isPremium ? (
        ...
        <div className="velum-card p-5 mb-8">
          <div className="flex items-center gap-3 mb-2">
            <Crown className="w-5 h-5 text-accent" />
            <p className="text-foreground text-sm font-sans font-medium">Premium Member</p>
          </div>
          <p className="text-ui text-xs">Monthly plan · Active</p>
        </div>
      )}
      ...
      {/* Sign out */}
      <button className="w-full velum-card-flat p-4 flex items-center justify-center gap-2 text-muted-foreground hover:text-foreground transition-colors mt-4">
